...
| Stripe Connect | external | — | charges, transfers, refunds, Express onboarding, webhooks |
...
- `paymentGateway() → {charge, transfer, refund, connectOnboardingLink}` — `NullGateway` when `STRIPE_SECRET_KEY` unset (charge auto-succeeds, refs `null_pi_*`), `StripeGateway` otherwise (PaymentIntent off-session; outcome via webhook; idempotency keys on every call). `constructStripeEvent(body, sig)` verifies webhook signatures.
...
See §3.1/§3.2. Click-wrap: agreement text rendered from terms at `v1`; acceptance timestamps (`venueAcceptedAt`, `performerAcceptedAt`) + template version on the booking row; accept route records both. **Requirements:** terms immutable after offer (change = new offer); all transitions audit to `events`; web maps domain errors to 4xx — never retries a 409 blindly. **Assumption to fix (§7.6):** acceptance should be gated on performer Stripe onboarding.
...
| A6 | Σ ledger = 0 per booking at terminal states; no transfer without a ledger row | requirement | ✅ in code; reconciliation job missing |
...
| A11 | Stripe Express is the sole payout path (tax delegation depends on it) | requirement | ✅ structurally; acceptance gate missing (§7.6) |
